...
## محل نگهداری پیش‌فرض: «انبار شماره ۱»
هر جا انبار مشخص نشده باشد، سیستم «انبار شماره ۱» را در نظر می‌گیرد:
- CLI: `--warehouse` در `stock-in`, `stock-out`, `request-purchase` و
  `--from` در `transfer` دیگر اجباری نیستند و پیش‌فرضشان همین انبار است.
- API: هر request بدون پارامتر `warehouse` روی همین انبار ثبت می‌شود
  (استثنا: `lowStock` که `warehouse` خالی در آن یعنی «همه‌ی انبارها»).
- پنل مدیریت: مبدا انتقال و انبار کاربر جدید از قبل روی همین انبار تنظیم‌اند
  و این گزینه همیشه در منوها هست، حتی اگر هنوز در تب Warehouses ثبت نشده باشد.
- کاربری که در تب Users انبارش خالی مانده، به همین انبار تعلق دارد.
- اولین باری که عملیاتی روی این انبار ثبت شود (یا `setup` اجرا شود)، ردیفش
  به‌صورت خودکار در تب Warehouses ساخته می‌شود.

⚠️ این نام در **سه جا** تعریف شده و هر سه باید مو‌به‌مو یکی بمانند، وگرنه
سیستم آن‌ها را دو انبار جدا می‌بیند:
`tools/sheets_client.py` (`DEFAULT_WAREHOUSE`)، `webapp/Code.gs`
(`DEFAULT_WAREHOUSE`)، `docs/assets/app.js` (`DEFAULT_WAREHOUSE`).
دقت کن که «انبار شماره ۱» (عدد فارسی) و «انبار شماره 1» (عدد لاتین) برای
این سیستم دو مقدار کاملاً متفاوت‌اند.
